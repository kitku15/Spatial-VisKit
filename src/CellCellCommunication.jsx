...
export default function CellCellCommunication({ n, r }) {
    ...
    async function loadData() {
      try {
        ...
        const cells = Array.from(
          new Set(edgesData.flatMap((d) => [d.source, d.target])),
        ).sort();
        const interactions = Array.from(
          new Set(edgesData.map((d) => d.interaction)),
        ).sort();
        const microenvs = Object.keys(microData).sort(
  ...
  useEffect(() => {
    ...
    if (plotData.length === 0) {
      container
        ...
        .attr(
          "class",
          "flex h-full items-center justify-center text-textMuted text-sm",
        )
    ...
    const names = Array.from(
      new Set(plotData.flatMap((d) => [d.source, d.target])),
    ).sort();
      ...
      interactions.forEach((intx) => {
        ...
        splitChords.push({
          source: {
            index: sIdx,
            startAngle: currS,
            endAngle: currS + sStep,
            value: intx.value,
          },
          target: {
            index: tIdx,
            startAngle: currT,
            endAngle: currT + tStep,
            value: intx.value,
          },
    ...
    svg
      ...
      .attr("fill", (d) => {
        if (colorBy === "Receiver")
          return colors[d.target.index % colors.length];
        if (colorBy === "Sender") return colors[d.source.index % colors.length];
        if (colorBy === "Interaction")
          return interactionColorScale(d.interaction);
      })
  ...
  const activeInteractions = Array.from(
    new Set(plotData.map((d) => d.interaction)),
  ).sort();
    ...
    <div className="p-6 flex flex-col gap-6 h-full bg-app">
              ...
              <div className="p-2 border-b border-borderLight bg-app">
                ...
                <div className="flex justify-between mt-2 px-1">
                  <button
                    type="button"
                    onClick={() =>
                      setSelectedInteractions(availableInteractions)
                    }
              ...
              <div className="overflow-y-auto p-2 flex flex-col gap-1">
                {availableInteractions
                  ...
                  .map((i) => (
                    <label
                      key={i}
                      className="flex items-center gap-2 text-sm text-textMain cursor-pointer hover:bg-app p-1 rounded transition-colors"
                    >
                      <input
                        type="checkbox"
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex-1 flex flex-col min-h-[600px] relative">
        <h3 className="font-bold text-lg mb-2 text-center text-textMain">
          Directed Interaction Network & Spatial Context
        </h3>
        <p className="text-sm text-textMuted text-center mb-4">
          Arrows indicate direction from Sender (Ligand) to Receiver (Receptor).
        </p>
        ...
        <div className="flex-1 w-full flex gap-4 overflow-hidden">
          <div className="flex-1 flex flex-col min-w-0 border border-borderLight rounded relative bg-panel overflow-hidden">
                ...
                <div className="absolute top-4 right-4 bg-panel/90 p-2 rounded shadow border border-borderLight max-h-[80%] overflow-y-auto z-10 backdrop-blur-sm">
                  ...
                  <div className="flex flex-col gap-1">
                    {activeInteractions.map((intx) => (
                      <div
                        key={intx}
                        className="flex items-center gap-2 text-xs text-textMain"
                      >
                        <span
                          className="w-3 h-3 rounded-full flex-shrink-0"
                          style={{
                            backgroundColor: interactionColorScale(intx),
                          }}
                        />
                        <span className="truncate max-w-[120px]" title={intx}>
                          {intx}
                        </span>
                      </div>
                    ))}
            ...
            <div className="h-1/3 min-h-[120px] max-h-[200px] border-t border-borderLight bg-app p-4 overflow-y-auto">
              <h4 className="text-lg font-bold text-textMuted uppercase tracking-wider mb-3">
                Participating Cell Types
              </h4>
              <div className="flex flex-wrap gap-x-4 gap-y-2">
                {cellColorMap.map((c) => (
                  <div
                    key={c.name}
                    className="flex items-center gap-2 text-lg text-textMain bg-panel px-2 py-1 rounded shadow-sm border border-borderLight"
                  >
                    <span
