...
function SearchableSelect({ options, value, onChange, placeholder }) {
  ...
  return (
    <div ref={wrapperRef} className="relative flex-1 max-w-[200px]">
      <div className="border border-borderMain bg-panel p-1 rounded flex items-center justify-between cursor-text" onClick={() => setIsOpen(true)}>
        <input
          type="text" className="outline-none w-full text-xs px-1 bg-transparent text-textMain"
        ...
        <div className="absolute z-10 w-full mt-1 bg-panel border border-borderMain rounded shadow-lg max-h-48 overflow-y-auto">
          {filteredOptions.length > 0 ? (
            filteredOptions.map((opt) => (
              <div key={opt.safe} className="p-2 text-xs text-textMain hover:bg-primary-light cursor-pointer" onClick={() => { onChange(opt); setSearch(""); setIsOpen(false); }}>
                {opt.original}
              </div>
            ))
          ) : (<div className="p-2 text-xs text-textMuted italic">No genes found</div>)}
        </div>
      )}
    </div>
  );
...
export default function DEAnalysis() {
  ...
  const clusterColorMap = useMemo(() => {
    ...
    const map = {};
    uniqueClusters.forEach((c, i) => map[c] = largeColorPalette[i % largeColorPalette.length]);
    return map;
  ...
  const createViolinPlot = (geneObj, sparseExprData) => {
    ...
    return sortedLabels.map((label) => ({
      y: clusters[label], name: label, type: 'violin',
      points: false, jitter: 0.5, pointpos: 0,
      line: { color: clusterColorMap[label] || themeColors.label }, 
      marker: { size: 3, opacity: 0.5 }, box: { visible: true }, meanline: { visible: true }
    }));
  };
    ...
    <div className="p-6 flex flex-col gap-4 h-full bg-app">
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex flex-wrap gap-6 items-center">
        <label className="text-sm font-semibold flex flex-col gap-1">
          <span className="text-textMuted uppercase tracking-wide text-xs">Annotation / Clustering</span>
          <select 
        ...
        <label className="text-sm font-semibold flex flex-col gap-1">
          <span className="text-primary-dark uppercase tracking-wide text-xs">Target Cluster (Vs Rest)</span>
          <select 
        ...
        <div className="ml-auto flex items-center">
          <InfoModal title={tabInfo.deAnalysis.title} content={tabInfo.deAnalysis.content} />
        </div>
      </div>
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="w-1/3 bg-panel border border-borderLight shadow-sm rounded p-4 flex flex-col">
          <h3 className="font-bold text-textMain text-center mb-1">Volcano Plot</h3>
          <p className="text-xs text-center text-textMuted mb-2 truncate">Cluster <b title={selectedCluster} className="text-textMain">{selectedCluster}</b> vs All Other Cells</p>
          <div className="flex-1 relative">
            {volcanoPlot ? (
              <Plot
                ...
                layout={{
                  autosize: true,
                  xaxis: { title: { text: 'Log2 Fold Change', standoff: 15 }, zeroline: true, zerolinecolor: themeColors.black, automargin: true },
              ...
              />
            ) : <div className="flex items-center justify-center h-full text-textMuted">Loading Volcano Data...</div>}
          </div>
        </div>
        ...
        <div className="w-2/3 flex flex-col gap-4">
          ...
          <div className="flex gap-4 h-1/2">
            <div className="w-full bg-panel border border-borderLight shadow-sm rounded p-3 flex flex-col">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-bold text-xs text-textMain mr-2">Gene 1:</h3>
          ...
          <div className="h-1/2 bg-panel border border-borderLight shadow-sm rounded flex flex-col overflow-hidden">
            <div className="bg-app border-b border-borderLight px-4 py-2">
              <h3 className="font-bold text-sm text-textMain">Top Marker Genes per Cluster</h3>
            </div>
            <div className="flex-1 overflow-auto p-0">
              <table className="w-full text-sm text-left">
                <thead className="bg-app sticky top-0 border-b border-borderMain z-10">
                  <tr>
                    <th className="px-4 py-2 text-textMain font-bold w-48">Cluster</th>
                    <th className="px-4 py-2 text-textMain font-bold">Top Genes</th>
                  </tr>
                </thead>
                <tbody>
                  {topGenesTable.map((row, i) => {
                    ...
                    const exactClusterKey = Object.keys(clusterColorMap).find(
                      k => k === clusterName || k === pythonCleanedName || k.trim().toLowerCase() === pythonCleanedName.trim().toLowerCase() || k.trim().toLowerCase() === clusterName.trim().toLowerCase()
                    ) || clusterName;

                    const color = clusterColorMap[exactClusterKey] || themeColors.neutral;
                    const isSelected = clusterName === selectedCluster;

                    return (
                      <tr key={i} className={`border-b border-borderLight hover:bg-primary-light ${isSelected ? 'bg-primary-light bg-opacity-50' : ''}`}>
                        <td className="px-4 py-2 font-semibold">
                          <div className="flex items-center gap-2">
                            ...
                            ></span>
                            <span className="truncate max-w-[150px] text-textMain" title={clusterName}>{clusterName}</span>
                          </div>
                        </td>
