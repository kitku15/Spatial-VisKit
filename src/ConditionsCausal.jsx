...
export default function ConditionsCausal() {
    ...
    fetch(`${API_BASE_URL}/${DATA_DIR}/causal_ccc/causal_metadata.json`)
      ...
      .then((data) => {
        ...
        if (comps.length > 0) {
          ...
          setSelectedComparison(firstComp);
          const pairs = data[firstComp] || [];
      ...
      .catch((err) => console.warn("Causal metadata not found.", err));
  }, []);

  // 2. Derive available sources
  const availableSources = useMemo(() => {
    if (!metadata || !selectedComparison) return [];
    const pairsForComp = metadata[selectedComparison] || [];
    return Array.from(new Set(pairsForComp.map((p) => p.source))).sort();
  }, [metadata, selectedComparison]);

  const effectiveSource = availableSources.includes(selectedSource)
    ? selectedSource
    : availableSources[0] || "";

  // 3. Derive available targets
  const availableTargets = useMemo(() => {
    if (!metadata || !selectedComparison || !effectiveSource) return [];
    const pairsForComp = metadata[selectedComparison] || [];
    return Array.from(
      new Set(
        pairsForComp
          .filter((p) => p.source === effectiveSource)
          .map((p) => p.target),
      ),
    ).sort();
  }, [metadata, selectedComparison, effectiveSource]);

  const effectiveTarget = availableTargets.includes(selectedTarget)
    ? selectedTarget
    : availableTargets[0] || "";

  // 4. Fetch the specific JSON file safely
  useEffect(() => {
    let isMounted = true;
    if (
      !metadata ||
    ...
    const pairInfo = metadata[selectedComparison]?.find(
      (p) => p.source === effectiveSource && p.target === effectiveTarget,
    );
    ...
    if (pairInfo && pairInfo.file) {
      fetch(`${API_BASE_URL}/${DATA_DIR}/causal_ccc/${pairInfo.file}`)
        ...
        .catch((err) => {
          ...
          if (isMounted) setCausalData(null);
        });
    }
    return () => {
      isMounted = false;
    };
  }, [selectedComparison, effectiveSource, effectiveTarget, metadata]);
  ...
  const lrPlotData = useMemo(() => {
    ...
    const hovertext = filtered.map(
      (d) =>
        `Source: ${d.source}<br>Target: ${d.target}<br>Ligand: ${d.ligand}<br>Receptor: ${d.receptor}<br>Stat: ${d.stat.toFixed(
          2,
        )}<br>P-val: ${d.pval.toExponential(2)}`,
    );
  ...
  const tfPlotData = useMemo(() => {
    ...
    const colors = x.map((val) =>
      val > 0 ? themeColors.danger : themeColors.primary,
    );
  ...
  useEffect(() => {
    ...
    if (causalData.network.nodes.length === 0) {
      container
        .append("div")
        .attr("class", "flex h-full items-center justify-center text-textMuted")
        .text("No intracellular network data available.");
      return;
    }
    ...
    node
      ...
      .style(
        "text-shadow",
        `0 1px 0 ${themeColors.paper}, 1px 0 0 ${themeColors.paper}, 0 -1px 0 ${themeColors.paper}, -1px 0 0 ${themeColors.paper}`,
      );
  ...
  if (!metadata) {
    return (
      <div className="p-6 text-textMuted flex items-center justify-center h-full">
        Loading Causal Network Data...
      </div>
    );
  }
    ...
    <div className="p-6 flex flex-col gap-4 h-full bg-app">
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex flex-wrap items-center gap-6">
        ...
        <label className="text-sm font-semibold flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const targets = Array.from(
                new Set(
                  pairs.filter((p) => p.source === src).map((p) => p.target),
                ),
              ).sort();
        ...
        <label className="text-sm font-semibold flex flex-col gap-1">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedSource(src);
              const pairs = metadata[selectedComparison] || [];
              const targets = Array.from(
                new Set(
                  pairs.filter((p) => p.source === src).map((p) => p.target),
                ),
        ...
        <div className="ml-auto flex items-center gap-4">
          ...
          <InfoModal
            title={tabInfo.causal.title}
            content={tabInfo.causal.content}
          />
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
        <div className="flex flex-col w-full lg:w-1/3 gap-4">
          <div className="bg-panel border border-borderLight shadow-sm rounded p-3 flex-1 flex flex-col min-h-0">
            <h3 className="font-bold text-sm text-textMain text-center mb-1">
              Condition-Altered Signals (LR)
            </h3>
            <p className="text-xs text-center text-textMuted mb-1 truncate">
              Sender ➔ Receiver
            </p>
        ...
        <div className="w-full lg:w-2/3 bg-panel border border-borderLight shadow-sm rounded flex flex-col overflow-hidden relative">
          <div className="bg-app border-b border-borderLight px-4 py-2 flex justify-between items-center z-10">
            <h3 className="font-bold text-sm text-textMain">
              Prior Knowledge Causal Network
            </h3>
