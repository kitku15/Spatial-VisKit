...
export default function QualityControl() {
  ...
  if (isLoading) return <div className="p-6 text-textMuted">Loading Fast QC Metrics...</div>;
  if (!histData || !thresholds) return <div className="p-6 text-danger">Failed to load QC JSONs.</div>;
  ...
  const makePlot = (metricKey, color, title, threshLines = []) => {
    ...
    if (!d) return <div className="text-textMuted flex items-center justify-center h-full">No data</div>;
  ...
  return (
    <div className="p-6 flex flex-col gap-6 h-full overflow-y-auto bg-app">
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex flex-wrap gap-4 justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-textMain">Pre-Filter Quality Control Metrics</h2>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <div className="bg-panel p-4 border border-borderLight shadow-sm rounded h-96 flex flex-col">
          <h3 className="font-bold text-textMain text-center mb-2">Total Transcripts per Cell</h3>
          <div className="flex-1 min-h-0">{makePlot('total_counts', themeColors.primary, 'Counts', [activeThresholds?.min_counts])}</div>
        </div>

        <div className="bg-panel p-4 border border-borderLight shadow-sm rounded h-96 flex flex-col">
          <h3 className="font-bold text-textMain text-center mb-2">Unique Genes per Cell</h3>
          <div className="flex-1 min-h-0">
            {makePlot('n_genes_by_counts', themeColors.success, 'Genes', [activeThresholds?.min_genes])}
          </div>
        </div>

        <div className="bg-panel p-4 border border-borderLight shadow-sm rounded h-96 flex flex-col">
          <h3 className="font-bold text-textMain text-center mb-2">Cell Area ({activeThresholds?.area_col})</h3>
          <div className="flex-1 min-h-0">{makePlot('area', themeColors.warning, 'Area', [activeThresholds?.min_area, activeThresholds?.max_area])}</div>
        </div>

        <div className="bg-panel p-4 border border-borderLight shadow-sm rounded h-96 flex flex-col">
          <h3 className="font-bold text-textMain text-center mb-2">{activeThresholds?.has_dapi ? 'Mean DAPI' : 'Nucleus Ratio'}</h3>
          <div className="flex-1 min-h-0">{makePlot('nucleus_signal', themeColors.info, 'Signal', [activeThresholds?.min_dapi])}</div>
        </div>
      </div>
    </div>
  );
