...
export default function MultiplexGeneOverlay() {
          ...
          setChannels([
            {
              id: 1,
              gene: geneList[0],
              color: defaultChannelColorNames[0],
              thresh: 5,
            },
            {
              id: 2,
              gene: geneList[1],
              color: defaultChannelColorNames[1],
              thresh: 5,
            },
          ]);
        }
  ...
  const addChannel = () => {
    ...
    const availColors = defaultChannelColorNames.filter(
      (c) => !usedColors.includes(c),
    );
    const nextColor =
      availColors.length > 0 ? availColors[0] : defaultChannelColorNames[0];

    setChannels((prev) => [
      ...prev,
      { id: Date.now(), gene: null, color: nextColor, thresh: 5 },
    ]);
  };
  ...
  const plotData = useMemo(() => {
    ...
    const activeChannels = channels.filter(
      (ch) => ch.gene && exprData[ch.gene.safe] && exprData[ch.gene.safe].i,
    );
    ...
    const channelSpecs = activeChannels.map((ch) => {
      ...
      return {
        dense,
        rgb: channelColorMap[ch.color] || [255, 255, 255],
        thresh: ch.thresh,
    ...
    for (let i = 0; i < locData.x.length; i++) {
      let cellSample =
        locData.sample && locData.sample[i] != null
          ? String(locData.sample[i])
          : "All";
      let cellSlide =
        locData.slide && locData.slide[i] != null
          ? String(locData.slide[i])
          : "All";
    ...
    return [
      {
        ...
        type: "scattergl",
        hoverinfo: "text",
        marker: { color: colors, size: pointSize, opacity: 0.9 },
      },
    ];
  ...
  if (isLoading) {
    return (
      <div className="p-6 text-textMuted">
        Loading spatial coordinates and expression matrix...
      </div>
    );
  }
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex flex-col gap-4 z-20">
          ...
          <div className="ml-auto flex items-center gap-6">
            <label className="flex flex-col text-xs font-semibold text-textMuted">
              ...
              <input
                ...
                className="cursor-pointer mt-1 accent-primary"
              />
            </label>
        ...
        <div className="flex flex-wrap gap-4 items-stretch">
          {channels.map((ch, idx) => (
            <div
              key={ch.id}
              className="flex flex-col gap-2 flex-1 min-w-[230px] max-w-[300px] bg-app p-3 border border-borderLight rounded relative shadow-sm"
            >
              {channels.length > 1 && (
                <button
              ...
              <h3 className="text-xs font-bold text-textMain uppercase tracking-wide">
                Channel {idx + 1}
              </h3>
              ...
              <div className="flex gap-2 items-center">
                ...
                >
                  {Object.keys(channelColorMap).map((c) => (
              ...
              <label className="text-xs text-textMuted flex flex-col mt-1">
                ...
                <input
                  ...
                  value={ch.thresh}
                  onChange={(e) =>
                    updateChannel(ch.id, "thresh", Number(e.target.value))
                  }
        ...
        {selectedSample === "__ALL__" ? (
          <div className="flex items-center justify-center h-full text-textMuted bg-app">
            <p className="p-6 text-center">
              ...
              Multiplex Overlay requires a specific sample to prevent coordinate
              overlap.
              <br />
          ...
          <div className="text-textMuted p-6">
            No data to display. Ensure genes are selected.
          </div>
        )}
      </div>
    </div>
