...
import { API_BASE_URL, DATA_DIR, themeColors, DYNAMIC_ANNOTATIONS, EXTRA_OBS_SETS } from "./config";
...
export default function TranscriptionFactor({ n, r, embedding, customColors={} }) {
  ...
  useEffect(() => {
    async function fetchHeatmap() {
      try {
        const res = await fetch(`${API_BASE_URL}/${DATA_DIR}/tf_heatmap_data.json`);
        if (!res.ok) return;
        const data = await res.json();
        setHeatmapData(data);
      ...
      <div className="bg-panel p-4 border border-borderLight shadow-sm rounded flex flex-wrap items-center gap-6">
        ...
        {viewMode === "Spatial" && (
            <div className="flex gap-4 border-l border-borderMain pl-4">
              ...
              <label className="text-sm font-semibold flex items-center gap-2 text-textMain">
                Sample:
                <select className="border border-borderMain rounded px-2 py-1 bg-panel font-normal disabled:opacity-50 outline-none focus:border-primary" value={selectedSample} onChange={(e) => setSelectedSample(e.target.value)} disabled={availableSamples.length <= 1}>
      ...
      <div className="flex gap-6 flex-1 min-h-0">
        <div className="flex-1 bg-panel border border-borderLight shadow-sm rounded flex flex-col overflow-hidden relative">
          <div className="bg-app border-b border-borderLight px-4 py-2 flex justify-between items-center z-10">
            <h3 className="font-bold text-sm text-textMain">TF Spatial Explorer</h3>
            <span className="text-xs text-textMuted">Select a TF from the right list to color cells</span>
          </div>
        ...
        <div className="w-[45%] bg-panel border border-borderLight shadow-sm rounded flex flex-col overflow-hidden">
          <div className="bg-app border-b border-borderLight px-4 py-2">
            <h3 className="font-bold text-sm text-textMain">TF Enrichment per Cell Type</h3>
          </div>

          <div className="p-2 border-b border-borderLight flex gap-2 overflow-x-auto">
            {allCellTypes.map((ct) => (
              <button
                key={ct} onClick={() => toggleCellType(ct)}
          ...
          <div className="flex-1 p-2 relative flex items-center justify-center">
            {!heatmapData ? (
              <span className="text-textMuted text-sm">Loading Heatmap Data...</span>
            ) : selectedCellTypes.length === 0 ? (
              <span className="text-textMuted text-sm">Select at least one cell type above.</span>
            ) : (
              <Plot
                data={filteredHeatmap}
                layout={{
                  autosize: true, margin: { l: 180, r: 20, t: 20, b: 100 }, xaxis: { tickangle: 45 }, yaxis: { automargin: true, autorange: "reversed" },
